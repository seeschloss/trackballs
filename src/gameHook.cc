...
GameHook::GameHook() {
  ...
  for(i=0;i<GameHookEvent_MaxHooks;i++)
    hooks[i]=NULL;
...
void GameHook::registerHook(GameHookEvent event,SCM hook) {
  ...
  if(hooks[event])
    scm_gc_unprotect_object(hooks[event]);
  /* Save this hook */
...
void GameHook::triggerHook(GameHookEvent event,SCM object) {
  ...
  if(hooks[event]) {
    ...
    if(a) subject = smobAnimated_make(a);
    else subject = smobGameHook_make(this);
...
void GameHook::tick(Real dt) {
  triggerHook(GameHookEvent_Tick,NULL);
}
