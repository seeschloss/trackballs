...
SCM_DEFINE (add_teleport, "add-teleport", 5, 0, 0,
	    ...
	    "Creates a teleporter at specified position, with specified destination. Returns an 'animated' object.")
#define FUNC_NAME s_add_teleport
{
  ...
  if(Game::current)
	return smobAnimated_make(new Teleport(scm_to_int(x),scm_to_int(y),
...
SCM_DEFINE (add_bird, "add-bird", 6, 0, 0,
	    ...
	    "Creates a bird at specified position, with specified destination. Returns an 'animated' object")
#define FUNC_NAME s_add_bird
{
  ...
  if(Game::current)
	return smobAnimated_make(new Bird(scm_to_int(x),scm_to_int(y),
                                          scm_to_int(dx),scm_to_int(dy),
                                          scm_to_double(size),
                                          scm_to_double(speed)));
  return SCM_UNSPECIFIED;
...
SCM_DEFINE (add_modpill, "add-modpill", 5, 0, 0,
			...
			"Adds a new modpill to level at x,y with given kind and resurrection state. Returns an 'animated' object")
...
/************** get-position-x ****************/
SCM_DEFINE (get_position_x, "get-position-x", 1, 0, 0,
	    (SCM obj),
	    "Gets the X position of the specified 'animed' object. Object must be of type animated, eg. a player, ball, etc.")
...
/************** get-position-y ****************/
SCM_DEFINE (get_position_y, "get-position-y", 1, 0, 0,
	    (SCM obj),
	    "Gets the Y position of the specified 'animated' object. Object must be of type animated, eg. a player, ball, etc.")
...
/************** get-position-z ****************/
SCM_DEFINE (get_position_z, "get-position-z", 1, 0, 0,
	    (SCM obj),
	    "Gets the Z position of the specified 'animated' object. Object must be of type animated, eg. a player, ball, etc.")
...
#define FUNC_NAME s_set_texture
{
  ...
  char *name=scm_to_locale_string(tname);
...
{
  SCM_ASSERT(IS_ANIMATED(obj),obj,SCM_ARG1,FUNC_NAME);
  SCM_ASSERT(SCM_NUMBERP(vx),vx,SCM_ARG2,FUNC_NAME);  SCM_ASSERT(SCM_NUMBERP(vy),vy,SCM_ARG3,FUNC_NAME);
...
SCM_DEFINE(set_bonus_level,"set-bonus-level",1,0,0,(SCM name),
		   "Makes level a bonus level and specified return level when this level is finished or player dies.")
{
  SCM_ASSERT(scm_is_string(name),name,SCM_ARG1,"set-bonus-level");  
  if(Game::current) {
	char *sname = scm_to_locale_string(name);
	if(strlen(sname) > 0) {
...
#define FUNC_NAME s_play_effect
{
  ...
  playEffect(scm_to_locale_string(name));
  return SCM_UNSPECIFIED;
}
#undef FUNC_NAME
...
/************* force-next-song ***********/
SCM_DEFINE (force_next_song, "force-next-song", 0, 0, 0,
	    (),
...
SCM_DEFINE (set_song_preference, "set-song-preference", 2, 0, 0,
	    (SCM song,SCM weight),
...
SCM_DEFINE (on_event, "on-event", 3, 0, 0,
	    (SCM event, SCM subject, SCM callback),
	    "Register a callback to be triggered when the given event occurs for the given object. Callback must accept two arguments (subject and object). Subject must be of type 'animated' or of type 'gameHook', object is only used for some events and is otherwise false.")
#define FUNC_NAME s_on_event
{
  ...
  SCM_ASSERT(SCM_FALSEP(callback)||SCM_NFALSEP(scm_procedure_p(callback)),callback,SCM_ARG3,FUNC_NAME);
...
SCM_DEFINE (get_event_callback, "get-event-callback", 2, 0, 0,
	    (SCM event, SCM subject),
