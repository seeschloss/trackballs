...
;; Give a hefty time bonus, but no points, for the baby balls
(default-on-death *score-baby* 0 10)

(set-start-position 252.5 252.5)
(start-time 150)

;; If we do this we force the game to first play the tb_sorrow song
;; (99% probability) and next the schizo song. Any old songs which
;; where already playing would be stopped when this level is started.
;(set-song-preference "tb_sorrow.ogg" 990)
;(set-song-preference "tb_schizophrenia.ogg" 1)
;(force-next-song)
...
(define ff
  (when (= (difficulty) *hard*)
    (let ((field (forcefield 251.5 250.5 0.0
                             2.0 0.0 0.0
                             0.5 *ff-kill*)))
      (switch 252.5 249.5
...
(time-on-death (add-flag 241 245 150 1 0.1) 5.0) ;; Where the black ball is
...
;; Test the new event callbacks by creating a mod-pill when we kill Mr. Black
(on-event
 *death*
 mr-black
 (lambda (subject object)
   (add-modpill (get-position-x subject) (get-position-y subject) *mod-extra-life* 30 -1)
   ))
